...
module.exports = function (options, callback) {
  ...
  async.parallel({
    ...
    if (err) {
      ...
      //
      // _active_tasks will ALWAYS be an Array, even an empty one
      // when nothing is running.
      //
      return ++(options.attempt) < maxTries
        ? setTimeout(module.exports.bind(null, options, callback), 10 * 1000)
        : callback(new Error('No active tasks returned from CouchDB'));
    }

    if (results.tasks && results.tasks.error) {
      return callback(new Error(results.tasks.error +' '+ results.tasks.reason));
    }
